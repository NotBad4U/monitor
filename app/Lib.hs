...
import Data.Map (Map(..), empty, fromList, union, difference, elems, fold)
import Debug.Trace
import Data.List ( (\\) )
...
data FLTL = TTrue | FFalse | Prop Char | FLTL :\/ FLTL |  FLTL :/\ FLTL | Not FLTL
    | X FLTL | Xweak FLTL | F FLTL | G FLTL | U FLTL FLTL | R FLTL FLTL
    deriving (Eq, Ord)
...
evlFLTL4 a (F p) = evlFLTL4 a (p :\/  Xweak (F p))
...
data OperatorTerm = 
    PropT Char 
    | And | Or | Gop | Fop | Xop | Xweakop | Uop | Rop | Nop
    | TrueTerm | FalseTerm deriving (Eq, Show, Ord)
...
δ4 (Prop p, a) | p == a = (Top, TTrue)
               | otherwise = (Bot, FFalse)
δ4 (Not (Prop p), a) | p /= a = (Top, TTrue)
                     | otherwise = (Bot, FFalse)
δ4 (X φ, a) = (PBot, φ)
δ4 (Xweak φ, a) = (PTop, φ)
δ4 (φ1 :/\ φ2, a) = (vφ1 ⊓ vφ2, simplfy (φ1' :/\ φ2'))
                    where (vφ1, φ1') = δ4(φ1, a)
                          (vφ2, φ2') = δ4(φ2, a)
δ4 (φ1 :\/ φ2, a) = (vφ1 ⊔ vφ2, simplfy (φ1' :\/ φ2'))
                    where (vφ1, φ1') = δ4(φ1, a)
                          (vφ2, φ2') = δ4(φ2, a)
...
mealy φ δ inputs = 
    let newTransitions = fromList $ fmap (\i -> ((φ, i), δ4 (φ, i))) inputs in
    let newStates = fmap snd (elems newTransitions) in
    let visitedStates = fmap snd (elems δ) in
    let unexploredStates = newStates \\  visitedStates in
    let t = fmap (\s -> mealy s (δ `union` newTransitions) inputs) unexploredStates in
        foldl union δ t
...
--     ((false,'p'),(B,false)),
--     ((false,'q'),(B,false)),
--     ((G 'q' and G not 'p' or X G 'q','p'),(B,false)),
--     ((G 'q' and G not 'p' or X G 'q','q'),(Tp,G 'q' and G not 'p' or X G 'q')),
--     ((G not 'p' or X G 'q','p'),(Bp,G 'q' and G not 'p' or X G 'q')),
--     ((G not 'p' or X G 'q','q'),(Tp,G not 'p' or X G 'q'))]