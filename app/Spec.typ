...
+ _Existential_ next requires the inclusion of a strong next operator, and
+ _Complementation by negation_ requires that a negated formula evaluates to the complemented and diﬀerent truth value
+ _Impartiality_ requires that a finite trace is not evaluated to ⊤ (⊥) if there still exists an infinite continuation leading to another verdict, and
+ _Anticipation_ requires that once every infinite continuation of a finite trace leads to the same verdict, then the finite trace evaluates to this very same verdict

= Development overview

The core idea is to develop a `monitor` based on the `FLTL₄` logic. We want to check 
if the implementation respect some invariant or temporal properties specified by their specification.

The `monitor` reads a stream of events output by an application and verifies after each new event whether a set of FLTL₄ formulae hold.

The `monitor` is a set of  well formed `FLTL₄` formulae and then compiled into a Mealy machine.
...
Let $Σ = 2^("AP")$ be the finite alphabet, p ∈ AP an atomic proposition, a ∈ Σ a letter. We define the syntax of the `FLTL₄` logic:
...
#align(center, diagram(spacing: (3.6em, 3em),
  mbox((0,0), $f$), mbox((1,0), $m$), mbox((2,0), $g$),
  edge((-1,0), (0,0), $a'$, "->"),
  edge((0,0), (1,0), $a$, "->"),
  edge((1,0), (2,0), $b$, "->"),
  edge((2,0), (3,0), $b'$, "->"),
...
#align(center, diagram(spacing: (4.5em, 3em),
  mbox((0,0), $m_φ$), mbox((1,0), $m_ψ$),
  edge((-1,0), (0,0), $Σ$, "->"),
...
#align(center, diagram(spacing: (4em, 2.4em),
  node((0,0), $Delta$), mbox((1,-1), $m_φ$), mbox((1,1), $m_ψ$),
