...
= Context
 ...
 @Bartocci2018IntroductionTR is a lightweight (yet rigorous) method that complements classical exhaustive verification techniques (such as model checking and theorem proving) with a more practical approach and try to be closer to the actual real system. RV works by analyzing the trace of the system’s actual execution, comparing it against a formal specification of the system behavior. This task is perform by a _monitor_ (cf. @fig-rv-loop) that run alongside a system to observe its behavior and determine if it satisfies a specified correctness property. The main advantage using RV can give precise information on the runtime behavior of the monitored system, without the pitfalls of developing models that require a re-implementation of the entire system in a modeling language.

Monitors can be classified as _offline_ and _online_ monitors @taxonomy-rv. Offline monitor @tla-rv process the traces generated by a system after the events, generally by reading the trace execution from a permanent storage system. Online monitors process the trace during the execution of the system. Online monitors are said to be synchronous if the processing of an event is attached to the system execution, blocking the system during the event monitoring. On the other hand, an asynchronous monitor has its execution detached from the system. Each type of monitor has a set of advantages. For example, offline monitors can be executed on different machines but require operations to save the log to a file. In contrast, synchronous online method can react at the exact moment a violation occurs.



This document aims to specify the development of a online monitor to verify if program guarantee some
temporal assertions defined in the Finite Temporal Linear Logic 4 (FLTL4) @rv-ltl@ltl-book, a suitable temporal logic for RV @rv-ltl. The @fig-rv-loop presents our approach.
A `monitor` is created from some formal FLTL4 `property`. The monitor is an Mealy machine equivalent
to the property. This is possible because all FLTL4 formulas can be transformed into a Mealy machine.
